...
// noop change to force redeploy: 2025-09-11T00:00:00Z
const corsHeaders = {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { telemetry, machine_id } = await req.json();
    console.log('Validating telemetry for machine:', machine_id);
    // Get recent telemetry history for pattern analysis
    const { data: recentData } = await supabase.from('telemetry_data').select('*').eq('machine_id', machine_id).order('timestamp', {
      ascending: false
    }).limit(50);
    const validation = await validateTelemetry(telemetry, recentData || [], supabase);
    // Store validation result
    await supabase.from('telemetry_validations').insert({
      machine_id,
...
async function validateTelemetry(current, history, supabase) {
  let anomaly_score = 0;
  const flags = [];
  ...
  if (history.length > 5) {
    ...
    if (lastTemp && currentTemp) {
      ...
      const timeDelta = (new Date(current.timestamp).getTime() - new Date(history[0].timestamp).getTime()) / 1000;
  ...
  const recentCount = history.filter((h)=>new Date(h.timestamp).getTime() > Date.now() - 60000).length;
  if (recentCount > 20) {
    anomaly_score += 25;
...
function generateMachineFingerprint(current, history) {
  let score = 0;
  const flags = [];
  // Check for identical consecutive readings (suggests simulation)
  if (history.length > 3) {
    const identical = history.slice(0, 3).every((h)=>h.data?.temperatures?.extruder === current.temperatures.extruder && h.data?.temperatures?.bed === current.temperatures.bed);
    if (identical) {
      score += 30;
  ...
  const temps = [
    current.temperatures.bed,
    current.temperatures.extruder
  ].filter(Boolean);
...
function calculateStatisticalOutliers(current, history) {
  let outlier_score = 0;
  const flags = [];
