...
serve(async (req) => {
  ...
  try {
    ...
    if (!recipientAddress || !mintAmount) {
      console.error('Missing required parameters:', { recipientAddress, mintAmount });
      return new Response(
        JSON.stringify({ 
          success: false, 
    ...
    const mintAuthorityKeypair = Keypair.fromSecretKey(
      new Uint8Array(JSON.parse(mintAuthorityKeypairJson))
    );
...
async function getUserIdFromWallet(supabase: any, walletAddress: string) {
